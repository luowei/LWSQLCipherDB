...
@implementation LWDBModel
+ (void)initialize {
    if (self != [LWDBModel class]) {
        [self createTable];
    }
}
...
+ (BOOL)isExistInTable {
    __block BOOL res = NO;
    LWSQLCipherDBTool *lkDB = [LWSQLCipherDBTool shareInstance];
...
- (BOOL)saveOrUpdate {
    ...
    LWDBModel *model = [self.class findByPK:primaryValue];
...
+ (BOOL)saveObjects:(NSArray *)array {
    //判断是否是JKBaseModel的子类
    for (LWDBModel *model in array) {
        if (![model isKindOfClass:[LWDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll {
    ...
    [lkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            LWDBModel *model = [[self.class alloc] init];
            for (int i = 0; i < model.columeNames.count; i++) {
...
+ (NSMutableArray *)PKAndColumnModify {
    ...
    for (int i = 0; i < properties.count; i++) {
        ...
        LWDBColumnDes *des = desDic[property];
        if (desDic[property]) {
